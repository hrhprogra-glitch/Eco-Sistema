...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const lineas = await query("SELECT * FROM cotizacion_lineas WHERE cotizacion_id = $1", [id]);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (lineas !== undefined) {
      await client.query("DELETE FROM cotizacion_lineas WHERE cotizacion_id = $1", [id]);
